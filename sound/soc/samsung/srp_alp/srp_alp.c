...
static void srp_prepare_buff(struct device *dev)
{
	...
	srp_info("[VA]DMEM[0x%p], [VA]commbox[0x%x]\n",
						srp.dmem, (unsigned int)srp.commbox);
}
...
static int srp_prepare_fw_buff(struct device *dev)
{
	...
	srp_info("obuf[0x%x],size-[%u]Bytes\n", (unsigned int)srp.sp_data.obuf,OBUF_SIZE);
