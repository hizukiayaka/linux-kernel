...
unsigned int exynos4_result_of_asv = 0;
...
static int exynos4x12_fuse_asv_store_result(struct samsung_asv *asv_info)
{
	...
	if (soc_is_exynos4412()) {
		if (samsung_rev() >= EXYNOS4412_REV_2_0) {
			for (i = 0; i < ARRAY_SIZE(exynos4x12_limit); i++) {
				if ((asv_info->ids_result <=
					exynos4x12_prime_limit[i].ids_limit) ||
		   			(asv_info->hpm_result <=
					exynos4x12_prime_limit[i].hpm_limit)) {
					exynos4_result_of_asv = i;
					break;
				}
			}
			...
			for (i = 0; i < ARRAY_SIZE(exynos4x12_limit); i++) {
				if ((asv_info->ids_result <=
					exynos4x12_limit[i].ids_limit) ||
		   			(asv_info->hpm_result <=
					exynos4x12_limit[i].hpm_limit)) {
					exynos4_result_of_asv = i;
					break;
				}
			}
		...
		pr_err("ASV: SoC is not Exynos4412.Seting DEFAULT_ASV_GROUP\n");

	if (exynos4_result_of_asv < DEFAULT_ASV_GROUP)
		exynos4_result_of_asv = DEFAULT_ASV_GROUP;

	pr_info("EXYNOS4X12: IDS : %d HPM : %d RESULT : %d\n",
		asv_info->ids_result, asv_info->hpm_result, exynos4_result_of_asv);
...
int exynos4x12_asv_init(struct samsung_asv *asv_info)
{
	...
	if ((tmp >> FUSED_SG_OFFSET) & 0x1) {
		 ...
		 * store 1 asv group into exynos4_result_of_asv.
		 */
		if (!exynos_orig_sp) {
			...
			if (exynos_cal_asv < DEFAULT_ASV_GROUP)
				...
				exynos4_result_of_asv = exynos_cal_asv;
		}
		pr_info("EXYNOS4X12(SG): ORIG : %d MOD : %d RESULT : %d\n",
			exynos_orig_sp, exynos_mod_sp, exynos4_result_of_asv);
