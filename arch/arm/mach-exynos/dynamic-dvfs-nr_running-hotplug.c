...
static unsigned int can_hotplug_status;
	...
	if (soc_is_exynos4412()) {
		if ((cpu_online(3) == 0) && (nr_running() >= 2) &&
		   ((freq_old >= freq_in_trg) && (freq_new >= freq_in_trg))) {
			if ((ctn_nr_running_over2 >= 3) &&
			   (ctn_freq_in_trg_cnt >= 3)) {
	...
	if (soc_is_exynos4412()) {
		if ((cpu_online(1) == 1) && (nr_running() < 4) &&
		   ((freq_old <= freq_out_trg) && (freq_new <= freq_out_trg))) {
			if ((ctn_nr_running_under4 >= 3) &&
			   (ctn_freq_out_trg_cnt >= 3)) {
				/* over 800ms for nr_running(), over 500ms for frequency, tunnable */
				cpu_down(1);
				ctn_freq_out_trg_cnt = 0;
			}
...
static int hotplug_dvfs_policy_transition(struct notifier_block *nb,
					     unsigned long val, void *data)
{
	...
	case CPUFREQ_ADJUST:
	case CPUFREQ_START:
		if (!strnicmp(policy->governor->name, "userspace",
			      CPUFREQ_NAME_LEN))
			can_hotplug = 0;
		else
			can_hotplug = 1;
...
static struct notifier_block hotplug_dvfs_policy = {
	.notifier_call = hotplug_dvfs_policy_transition,
};
					...
					unsigned long val, void *data)
{
	...
	case PM_SUSPEND_PREPARE:
		can_hotplug_status = can_hotplug;
		can_hotplug = 0;
		ctn_freq_in_trg_cnt = 0;
		ctn_freq_out_trg_cnt = 0;
		break;
	...
	case PM_POST_SUSPEND:
		can_hotplug = can_hotplug_status;
		break;
	}
...
static int __init exynos4_integrated_dvfs_hotplug_init(void)
{
	...
	if (!strnicmp(policy.governor->name, "userspace", CPUFREQ_NAME_LEN))
		can_hotplug = 0;
	else
		can_hotplug = 1;
	...
	cpufreq_register_notifier(&hotplug_dvfs_policy,
...
late_initcall_sync(exynos4_integrated_dvfs_hotplug_init);
