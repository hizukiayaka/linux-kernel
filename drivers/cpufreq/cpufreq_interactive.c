...
static void cpufreq_interactive_idle_start(void)
{
	...
	if (!down_read_trylock(&pcpu->enable_sem))
		return;
	if (!pcpu->governor_enabled || pcpu->cpu_hotplug_out) {
		up_read(&pcpu->enable_sem);
		return;
	}
...
static int cpufreq_interactive_hotplug_notifier(
	struct notifier_block *nb, unsigned long val, void *data)
{
	...
	case CPU_DOWN_FAILED:
		cpufreq_interactive_timer_resched(pcpu);
		pcpu->cpu_hotplug_out = false;
		break;
	case CPU_DOWN_PREPARE:
		pcpu->cpu_hotplug_out = true;
...
static struct notifier_block hotplug_notifier_block = {
	.notifier_call = cpufreq_interactive_hotplug_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		register_cpu_notifier(&hotplug_notifier_block);
		mutex_unlock(&gov_lock);
		break;
	...
	case CPUFREQ_GOV_STOP:
		...
		unregister_cpu_notifier(&hotplug_notifier_block);
