/* linux/drivers/media/video/fimg2d/fimg2d4x_blt.c
 *
...
{
	int ret = 0;
	struct device *sysmmu = NULL;
	struct sysmmu_drvdata *sysmmu_data;
	struct iommu_domain *sysmmu_domain;
	...
	sysmmu = ctrl->dev->archdata.iommu;
	sysmmu_data = dev_get_drvdata(sysmmu);
	if (!sysmmu_data)
		fimg2d_err("failed to get sysmmu_data\n");

	sysmmu_domain = ctrl->dev->archdata.mapping->domain;
	...
	while (1) {
		...
		if (addr_type == ADDR_USER || addr_type == ADDR_USER_CONTIG) {
			...
			exynos_sysmmu_enable(sysmmu_data,
