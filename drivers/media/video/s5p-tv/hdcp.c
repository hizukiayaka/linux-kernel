...
#define HDMI_TRANS_EVERY_SYNC 2
#define HDMI_DO_NOT_TANS 0


#undef tvout_dbg
/* #define CONFIG_HDCP_DEBUG */
...
void s5p_hdmi_reg_intc_enable(int intr, u8 en)
{
	u8 reg;
	reg = readb(hdmi_base + HDMI_INTC_CON);

	if (en) {
		if (!reg)
			reg |= HDMI_INTC_EN_GLOBAL;
	...
	writeb(reg, hdmi_base + HDMI_INTC_CON);

}

u8 s5p_hdmi_reg_get_hpd_status(void)
{
	return readb(hdmi_base + HDMI_HPD_STATUS);
}

bool s5p_hdmi_ctrl_status(void)
{
	return readb(hdmi_base + HDMI_CON_0) & 0x01 ? true : false;
}

void s5p_hdmi_reg_audio_enable(u8 en)
{
	u8 con, mod;
	con = readb(hdmi_base + HDMI_CON_0);
	mod = readb(hdmi_base + HDMI_MODE_SEL);
	...
	if (en) {
		...
		con |= HDMI_ASP_MASK;
		writeb(HDMI_TRANS_EVERY_SYNC, hdmi_base + HDMI_AUI_CON);
	} else {
		con &= ~HDMI_ASP_MASK;
		writeb(HDMI_DO_NOT_TANS, hdmi_base + HDMI_AUI_CON);
	}

	writeb(con, hdmi_base + HDMI_CON_0);
}

void s5p_hdmi_reg_bluescreen(bool en)
{
	u8 reg = readl(hdmi_base + HDMI_CON_0);
	hdmi_bit_set(en, reg, HDMI_BLUE_SCR_EN);
	writeb(reg, hdmi_base + HDMI_CON_0);
}
...
static int s5p_hdcp_encryption(bool on)
{
	...
	writeb(reg, hdmi_base + HDMI_ENC_EN);
...
static int s5p_hdcp_read_bcaps(void)
{
	...
	if (!s5p_hdmi_reg_get_hpd_status() || s5p_hdmi_ctrl_status() == false \
		 || on_stop_process)
		goto bcaps_read_err;
...
void s5p_hdmi_reg_sw_hpd_enable(bool enable)
{
	...
	reg = readb(hdmi_base + HDMI_HPD);
	reg &= ~S5P_HDMI_HPD_SEL_I_HPD;

	if (enable)
		writeb(reg | S5P_HDMI_HPD_SEL_I_HPD, hdmi_base + HDMI_HPD);
	else
		writeb(reg, hdmi_base + HDMI_HPD);
}
...
static void s5p_hdcp_reset_sw(void)
{
	...
	s5p_hdmi_reg_intc_enable(HDMI_INTC_EN_HPD_PLUG, 0);
	s5p_hdmi_reg_intc_enable(HDMI_INTC_EN_HPD_UNPLUG, 0);
	...
	if (reg & 1 << HDMI_INTC_EN_HPD_PLUG)
		s5p_hdmi_reg_intc_enable(HDMI_INTC_EN_HPD_PLUG, 1);
	if (reg & 1 << HDMI_INTC_EN_HPD_UNPLUG)
		s5p_hdmi_reg_intc_enable(HDMI_INTC_EN_HPD_UNPLUG, 1);
...
static void s5p_hdcp_reset_auth(void)
{
	...
	reg = readb(hdmi_base + HDMI_STATUS_EN);
	reg &= S5P_HDMI_INT_DIS_ALL;
	writeb(reg, hdmi_base + HDMI_STATUS_EN);
...
static int s5p_hdmi_start_encryption(void)
{
	...
	do {
		reg = readb(hdmi_base + HDMI_SYS_STATUS);
...
int s5p_hdcp_stop(void)
{
	...
	sfr_val = readb(hdmi_base + HDMI_STATUS_EN);
	sfr_val &= S5P_HDMI_INT_DIS_ALL;
	writeb(sfr_val, hdmi_base + HDMI_STATUS_EN);

	sfr_val = readb(hdmi_base + HDMI_SYS_STATUS);
	sfr_val |= S5P_HDMI_INT_EN_ALL;
	writeb(sfr_val, hdmi_base + HDMI_SYS_STATUS);
...
int s5p_hdcp_start(struct hdmi_device *hdev)
{
	u32  sfr_val;
	...
	writeb(S5P_HDMI_INT_EN_ALL, hdmi_base + HDMI_STATUS_EN);
