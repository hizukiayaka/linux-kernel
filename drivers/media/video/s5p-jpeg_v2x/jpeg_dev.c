...
static int jpeg_m2m_open(struct file *file)
{
	...
	enum jpeg_node_type node;
  	...
  	err = exynos4_busfreq_lock(0);
	if(err){
		jpeg_err("cannot acquire freq lock\n");
		return err;
	}
#endif
...
#ifdef CONFIG_PM_RUNTIME
	err = pm_runtime_get_sync(&dev->plat_dev->dev);
	if (err < 0)
		return err;
	err = platform_sysmmu_on(&dev->plat_dev->dev);
	if (err < 0) {
...
#endif
#ifdef CONFIG_PM_RUNTIME
	platform_sysmmu_off(&ctx->dev->plat_dev->dev);
	...
	if (enc_param.in_plane == 1) {
		src_addr2 = src_addr + enc_param.in_width*enc_param.in_height;
			/*Here src_addr2 is the start address of U/V-component 
 			of 420_1P image. Because JPEGv2x supports only 420_2P and 420_3P,
			We are processing 420_1P as a 420_2P image.
			Address of U/V-component will go as the Address of 2nd Plane*/
...
static int jpeg_resume(struct platform_device *pdev)
{
	...
	if (err < 0)
		return err;
	err = platform_sysmmu_on(&pdev->dev);
	if (err < 0) {
		pm_runtime_put_sync(&pdev->dev);
		return err;
	}
...
#endif
	if (jpeg_drv->vb2 && jpeg_drv->vb2->suspend)
...
#endif
	if (jpeg_drv->vb2 && jpeg_drv->vb2->resume)
